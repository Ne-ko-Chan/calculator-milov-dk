...
def test_ignore_space():
    res = subprocess.run(
        ["build/app.exe","--float"], input="10                                                                                                  -9\t/\f\n2 + 5\r", text=True, capture_output=True
    )
    assert res.returncode == 0
    assert (float(res.stdout) - 10.5) <10**(-4)
...
def test_mismatched_braces1():
    res = subprocess.run(
        ["build/app.exe","--float"], input="10 + 5 - (6 * 9))", text=True, capture_output=True
    )
    assert res.returncode != 0
...
def test_double_op():
    ...
    assert res.returncode != 0
    res = subprocess.run(
        ["build/app.exe","--float"], input="10 + 5 - (6 * 9) //3", text=True, capture_output=True
    )
    assert res.returncode != 0
